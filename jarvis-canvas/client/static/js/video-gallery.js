...
function renderGallery() {
    ...
    gallery.innerHTML = filteredVideos.map((vid, index) => {
        ...
        const provider = vid.provider || detectProvider(vid.name);
...
function detectProvider(name) {
    // Detect provider from filename patterns
    const lower = name.toLowerCase();
    
    // Check for explicit provider markers
    if (lower.includes('gemini') || lower.includes('veo')) return 'Gemini Veo';
    if (lower.includes('runway')) return 'Runway';
    if (lower.includes('pika')) return 'Pika';
    if (lower.includes('kling')) return 'Kling';
    
    // Default to xAI for video_ prefix (generated by image-to-video tool)
