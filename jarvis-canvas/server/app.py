...
from config import GENERATED_AUDIO_DIR, GENERATED_IMAGES_DIR, GENERATED_VIDEOS_DIR
...
def create_app(mode='cloud'):
    ...
    app.register_blueprint(audio_actions_bp)
    ...
    # Browser-facing routes (/, /gallery, /video-gallery, /audio-gallery) require auth
...
def run_server(host='0.0.0.0', port=8890, mode='cloud', debug=False):
    ...
    audio_count = len([f for f in GENERATED_AUDIO_DIR.iterdir()
                      if f.is_file() and f.suffix.lower() in ('.mp3', '.wav', '.ogg', '.opus', '.m4a', '.aac', '.flac')]) if GENERATED_AUDIO_DIR.exists() else 0
    ...
    audio_gallery_url = f"{public_url}/audio-gallery"
...
║  Audio Gallery: {audio_gallery_url:<42} ║
╠═══════════════════════════════════════════════════════════════╣
