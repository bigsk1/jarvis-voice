"""Explicit trusted binding for the Phase 3a loopback HTTP service protocol.
...
class SubmissionRejected(TaskError):
    """The local service returned a response proving it did not accept work."""
...
class LocalCallbackRunner:
    def __init__(self, service, bindings, *, prepare=None):
    ...
    def __call__(self, context):
        ...
        try:
            source_id, parameters = self.bindings[tool]
            ...
            extra, headers = self.prepare(context) if self.prepare else ({}, {})
            context.checkpoint()
        ...
        try:
            url = endpoint(submission.pop('submit_url'), local_only=True)
            ...
            # A 4xx from this loopback service proves that no work was accepted.
            raise KnownFailure({'ok': False, 'speech':
                'Background work could not start because the local service rejected the request.'}) from exc
        except Exception as exc:
