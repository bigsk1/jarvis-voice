...
class ToolExecutor:
    ...
    def _get_subprocess_timeout(self, tool_name: str) -> int:
        ...
        if tool_name == "serpapi_amazon_search":
            return 90  # SerpApi include product details can take up to 90 seconds
        ...
        if tool_name == "serpapi_google_shopping_light":
            return 120  # One Google Shopping Light request with a 90s HTTP allowance
        if tool_name == "serpapi_google_immersive_product":
            return 120  # One Google Immersive Product request with a 90s HTTP allowance
        ...
        if tool_name == "serpapi_open_table_reviews":
            return 90  # One OpenTable Reviews request with a 45s HTTP allowance
