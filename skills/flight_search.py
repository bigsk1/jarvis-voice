...
# fast-flights only encodes route, date, cabin, passengers, airlines, and max
# stops into its query. These other filters have no fallback equivalent and
# are reported back as unapplied.
...
def search_fallback(options: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if "," in options["departure_id"] or "," in options["arrival_id"]:
        raise ValueError(
            "The keyless flight fallback accepts one departure and one arrival airport. "
            "Search each nearby-airport pair separately, or configure SERP_API_KEY for a combined search."
        )

    airline_codes = [
        code.strip()
        for code in (options["include_airlines"] or "").split(",")
        if code.strip()
    ]
    query_filters = {
        "max_stops": STOPS_MAX[options["stops"]],
        "airlines": airline_codes or None,
    }
    legs = [
        FlightQuery(
    ...
    for attempt_number, proxy_url in enumerate(
        build_proxy_url_attempts(direct_fallback_default=True), start=1
    ):
        attempt_label = "direct" if proxy_url is None else f"proxy {attempt_number}"
        try:
            entries = get_flights(query, proxy=proxy_url)
            break
        except Exception as exc:
            # Client exceptions may echo authenticated proxy URLs. Keep tool
            # output useful without exposing credentials from configuration.
            attempt_errors.append(f"{attempt_label}: {type(exc).__name__}")
    if entries is None:
        details = "; ".join(attempt_errors) or "no connection attempts were available"
        raise RuntimeError(f"Keyless Google Flights search failed ({details}).")

    results = [convert_fallback_itinerary(entry, options["travel_class"]) for entry in entries]

    # Price is applied locally because fast-flights has no query-time price cap.
    # Unknown prices cannot truthfully satisfy a user-specified maximum.
    if options["max_price"] is not None:
        results = [
            item for item in results
            if item.get("price") is not None and item["price"] <= options["max_price"]
        ]
