...
def build_synthetic_full_transcript(user_query: str) -> str:
    ...
    lines = [
        ...
        "Lines tagged active_pref are resolved canonical response preferences.",
        "Current user request wins, then active_pref, then Profile Card/memory/Intelligence; never merge conflicts.",
        "Other lines are semantic or exact-keyword matches; use when relevant and ignore if off-topic.",
        "Freshness note: For live market/weather questions, newer live tool calls outrank older stored memory.",
        "",
        "- response_style: example value (memory_id: 1, category: preference, active_pref slot=response_style scope=persistent rank=1.20, saved_at: 2026-01-01 12:00:00, age: 60m)",
        "===",
        "",
