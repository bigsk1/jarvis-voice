...
> **Snapshot note:** This is a self-contained vendored reference. Links to
> pages that were not copied into this repository have been converted to plain
> type references; use the versioned Crawl4AI documentation for material
...
See the `arun_many()` examples in this snapshot for multi-URL crawling.
...
```
Below is an organized look at the parameters that can go inside `CrawlerRunConfig`, divided by their functional areas. For **Browser** settings (e.g., `headless`, `browser_type`), see the `BrowserConfig` sections in this snapshot.
## 1. Core Usage
```python
from crawl4ai import AsyncWebCrawler, CrawlerRunConfig, CacheMode
...
# `arun_many(...)` Reference
> **Note**: This function is very similar to `arun()` but focused on **concurrent** or **batch** crawling. If you’re unfamiliar with `arun()` usage, review its section in this snapshot first.
## Function Signature
```python
async def arun_many(
...
## Conclusion
Use `arun_many()` when you want to **crawl multiple URLs** simultaneously or in controlled parallel tasks. If you need advanced concurrency features (like memory-based adaptive throttling or complex rate-limiting), provide a **dispatcher**. Each result is a standard `CrawlResult`, possibly augmented with concurrency stats (`dispatch_result`) for deeper inspection.
...
**What**: If `fetch_ssl_certificate=True` in your CrawlerRunConfig, **`result.ssl_certificate`** contains an **`SSLCertificate`** object describing the site's certificate. You can export the cert in multiple formats (PEM/DER/JSON) or access its properties like `issuer`,
...
In most examples, you create **one** `BrowserConfig` for the entire crawler session, then pass a **fresh** or re-used `CrawlerRunConfig` whenever you call `arun()`. This tutorial shows the most commonly used parameters; consult the versioned upstream reference for rare fields.
...
```
## 5. Next Steps
- `BrowserConfig`, `CrawlerRunConfig`, and `LLMConfig` reference sections
...
The `arun()` method returns a `CrawlResult` object with several useful properties. Here's a quick overview:
```python
config = CrawlerRunConfig(
    markdown_generator=DefaultMarkdownGenerator(
...
Below are the key interaction-related parameters in `CrawlerRunConfig`. Consult the versioned upstream reference for the full list.
