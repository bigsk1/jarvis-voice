...
- Catalog default: `gpt-image-2.5-sunburst` (optional pin: `OPENAI_IMAGE_MODEL`; use `gpt-image-2.5-flare` for faster everyday generation)
- Best text rendering and instruction following
- Sizes: legacy models use 1024x1024, 1536x1024, 1024x1536; GPT Image 2 and 2.5 support flexible 1K/2K/4K dimensions up to 3840px on the long edge
- Quality: low, medium, high (mapped from 1K/2K/4K)
- Transparent backgrounds (png/webp only) on GPT Image 2.5, `gpt-image-1.5`, `gpt-image-1`, and `gpt-image-1-mini`; `gpt-image-2` does not support transparent backgrounds
