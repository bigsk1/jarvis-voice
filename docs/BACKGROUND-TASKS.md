...
Web task management uses one supervised skill runner with two reviewed policies: `local_skill_v1` for `convert_file`, and `remote_skill_v1` for `generate_image`, `generate_video`, `generate_music`, and `create_social_clip`. Tools enabled in **Settings → Tools → Background tasks** return an acceptance receipt, release the chat turn, run in a separate worker, and deliver a saved card and follow-up answer to the same conversation. You can send another message and call another tool while the job runs.
...
Local conversion needs no Tailscale network, VPS, or public endpoint. Incoming callbacks, Samantha, and the standalone relay remain later phases. OpenCode and `/api/alerts` are outside this work. Firefox source compatibility is tested in-tree; packaging and AMO release remain separate steps.
...
Defaults: two globally running jobs, two per adapter, five outstanding jobs per conversation generation, and 100 queued jobs. Result retention defaults to 30 days (configurable from 1 to 3650). Arguments, progress, and results are bounded. Workers reconcile on startup, poll every second with jitter, and heartbeat every five seconds with a 30-second lease. PID files do not confer execution authority.
...
The normal native `bin/start`, `bin/start --ui-only`, and `bin/start --no-api` groups also start **jarvis-task-worker** in tmux. Existing aliases for those commands need no changes. `bin/start --stop` stops that worker with the other sessions. One worker serves both cloud and local jobs, including when Web changes modes. Starting a worker never enables admission or adds tools to the saved allowlist.
...
The dashboard's Services tab includes Start Task Worker, Stop Task Worker, Task Worker Status, and Task Worker Logs. Both the start groups and dashboard delegate lifecycle management to `bin/jarvis-task-worker start --tmux` / `stop --tmux`. The worker command owns readiness, session creation, graceful shutdown, and status reporting; `bin/start` only groups the services. `restart --tmux` restarts the managed session, and `status --tmux` prints the native status row. Plain start/stop/restart retain their systemd meaning.
...
The local policy requires `dangerous: false`, `auto_approve: true`, and `network: false`, plus the reviewed `completion_scope: "process_group"` declaration. Review child processes, daemonization, side effects, timeout/resource needs, and cancellation evidence before adding a binding. A local process ending cannot prove that a submitted remote operation stopped; the remote policy below handles this uncertainty. OpenCode, Samantha, and incoming callbacks remain deferred. Resource limits and permission metadata are not an OS network or filesystem sandbox.
...
Incoming callbacks have a separate local HTTP test gate in Phase 3a covering middleware, body limits, authentication, and a durable inbox. Samantha or another provider is added only after a completion POST contract exists and is verified. The optional public relay remains a separate deployable service; none of these integrations is required for local background execution.
