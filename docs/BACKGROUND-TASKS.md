...
Local conversion needs no Tailscale network, VPS, or public endpoint. [Task callbacks](TASK-CALLBACKS.md) add an optional authenticated inbox and **Settings → Integrations** controls, proven with a separate local HTTP service. The opt-in [Browser Use](BROWSER-USE.md) tool is the first callback binding; Samantha and the standalone relay remain later phases. OpenCode and `/api/alerts` are outside this work. Firefox source compatibility is tested in-tree; packaging and AMO release remain separate steps.
...
Incoming callbacks pass the separate [Phase 3a local HTTP gate](TASK-CALLBACKS.md#isolated-acceptance-proof), covering middleware, body limits, authentication, durable inbox and one saved answer after another Web tool turn. Samantha or another provider is added only after its completion POST contract exists and is verified. The optional public relay remains future work as a separate deployable service; none of these integrations is required for local background execution.
