...
### 1) Action-Level Permission and Preflight Policies (Optional)
**Priority:** Deferred / narrow high-risk use only

Foreground Jarvis Web chat now pauses on an exact prepared call when its tool manifest explicitly sets `auto_approve: false`. Web Talk and Firefox Companion present the same pending call with buttons. Other entry points, including workflows and the separate FastAPI surface, retain their current behavior. See [Tool Calling System](TOOL_CALLING_SYSTEM.md#permission-system).

A future policy could distinguish read-only and mutating actions within one tool. Any new adapter should preserve the current exact-call, one-shot, short-expiration boundary rather than ask approval before the model has chosen arguments.
...
- Tool-specific validation and preflight before the Web approval prompt
- Tool schemas — action-level risk metadata if coarse tool-level permissions become insufficient
- Separate entry-point adapters only if their default approval policy changes
