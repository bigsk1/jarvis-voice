...
This short framebuffer demonstration shows the face emerging from the Matrix rain, reacting while active, speaking with audio-driven mouth movement, and dissolving back into the rain.
...
- **THINK scanline.** In real `THINK` a ±2.5-row band sweeps top to bottom once every 1.2 s. `JARVIS_HEAD_SCAN_LEVELS` / `--scan-levels` is its signed 256-level ramp offset (-255 to 255, default +72): positive brightens, `0` hides the band, and negative produces a dark reverse sweep. It is deliberately independent of face brightness, so the two can be tuned separately. `LISTEN` and `THINK` no longer look identical. `--demo-think` shows it without a wake.
- **Optional ambient scan.** The current wake path emits `listen` and `sleep`, not `think`, so `JARVIS_HEAD_AMBIENT_SCAN=true` can reuse the framebuffer scan as visual choreography without forging a state event. The first sweep starts 3 seconds after the face becomes visible; after each primary sweep, the next waits a random 10–14 seconds. Each primary has a 15% chance of a second sweep 0.2 seconds later, and that second pass jumps the face by one nearby cell for a brief glitch. It continues while the mouth is speaking, resets whenever the face disappears, and is repeatable under `--seed`. A real `think` event still uses the continuous semantic scan and takes precedence. The timing and double chance have their own environment keys below; the feature defaults off on fresh clones.
...
Deferred-until-panel tuning knobs are all module constants at the top of `fb_render.py` (`REVEAL_WIDTH`, `BREATH_LEVELS`, `RIPPLE_LEVELS`, `SKIN_GLOW_DECAY`, `AFTERGLOW_LEVELS`, `RAIN_AFTERGLOW_DECAY`, `SCAN_*`, `SPEECH_*`) and `app.py` (`EYE_DISTANCE_WEIGHT`, `BREATH_PERIOD_SECONDS`). Decay factors are per frame and tuned at 30 FPS. Brightness and presence remain the two main look knobs; the panel-tuned veiled look is `--face-presence 0.45 --face-brightness 0.4`. Ambient-scan timing is environment-only because it is installation choreography, not a per-demo rendering control.
...
# JARVIS_HEAD_SCAN_LEVELS=72  # -255..255; positive brightens, 0 hides, negative darkens
# Optional visual-only fb scan while a face is visible; does not change THINK state
# JARVIS_HEAD_AMBIENT_SCAN=false
...
`bin/kiosk.sh` forwards explicit renderer, face, scan, and ambient-scan `JARVIS_HEAD_*` settings from the launching shell into the clean session environment and otherwise lets the launcher hydrate them from the selected mode file. Head flags after `--` still win over both. Preflight resolves the *effective* renderer, device, brightness, presence, and signed scan offset the same way the launcher will (the head flag in either spelling, `--scan-levels -48` or `--scan-levels=-48`, then environment/config, then the default). It also validates the ambient boolean, finite timing windows, ordered minimum/maximum, and 0–1 double chance. It refuses to create the unit when any of those are invalid, when the fb renderer is selected but the device is not a character device, when the renderer name is unknown, when `--snapshot` is passed, when the scan offset is not an integer from -255 to 255, or when the effective face brightness is outside 0.2–1.5 or presence outside 0.3–1.0. So `JARVIS_HEAD_SCAN_LEVELS=999 ./bin/kiosk.sh start -- --scan-levels -48` starts (the valid flag wins) and `./bin/kiosk.sh start -- --scan-levels 999` is refused here rather than dying after "started". The failure is printed synchronously where the operator can read it instead of the transient unit dying after "started".
...
| 8 | Choreography on fb: features-first brightness-lerp coalesce, breathing, rain-through-skin ripple, phosphor trail, semantic and optional ambient scanlines, speech-energy jaw pulse, `--face-brightness`, `--face-presence` | On the panel the face condenses out of the rain around the eyes and releases eyes-last, with no scattered pop; idle has visible breath; leads crossing the face light it; think and listen are distinguishable; optional ambient sweeps work during listen and speech without changing state; loud speech moves the jaw; compose stays within ~1 ms of Phase 7; curses unchanged except reveal order |
...
Ambient choreography follow-up (2026-09-02): live wake does not emit `think`, so the framebuffer can now schedule an optional visual-only sweep while any face is visible, including during speech. The default timing is first sweep after 3 s, then a random 10–14 s pause; 15% of primary sweeps get a second pass after 0.2 s with one nearby-cell glitch nudge. It resets on hide, uses its own seeded RNG, never updates the state machine or activity lease, and a real `think` continues to own the scan phase. Fresh-clone examples default it off; this host enables it in both ignored mode files. The signed `--scan-levels` / `JARVIS_HEAD_SCAN_LEVELS` control was added after panel-tuning discussion so the sweep can be bright, invisible, or dark without following the face gain. Tests cover timing, hide/reset, deterministic scheduling, double/nudge integration, simultaneous speech energy, semantic precedence, signed scan ordering and bounds, config hydration/forwarding, CLI precedence, and synchronous kiosk rejection of invalid settings. 188 focused tests.
...
# Same receiver with occasional visual-only scan/glitch choreography.
JARVIS_HEAD_AMBIENT_SCAN=true ./bin/kiosk.sh start -- --renderer fb

# Tune either semantic THINK or ambient sweeps independently of face brightness.
./bin/kiosk.sh start -- --renderer fb --scan-levels 36    # subtle bright sweep
./bin/kiosk.sh start -- --renderer fb --scan-levels -48   # dark reverse sweep
./bin/kiosk.sh start -- --renderer fb --scan-levels 0     # no visible band
```
...
The wrapper reads optional settings from the selected `cloud.env` or `local.env`; explicit command-environment values win. Ambient scan settings are read when the display starts, so stop and start the kiosk after changing them. It refuses `tty1` and a target with an active getty. It does not enable itself at boot.
...
- Phase 8: eye anchors and eyes-first thresholds, breath bounds, `VisemeTimeline.levels` extraction and validation, `mouth_energy`, deterministic ambient-scan timing/reset/double behavior, and on the compositor the lerp ordering, phosphor trail, skin ripple decay, signed semantic/ambient scan and speech-energy placement with the mouth hole intact, face-brightness gain bounds, and independent scan-level bounds; the presence cap and mouth anchor; `kiosk.sh` forwarding, CLI precedence, and preflight of the face, scan, and ambient settings; the `chvt` and `KD_TEXT` restore-failure warnings
