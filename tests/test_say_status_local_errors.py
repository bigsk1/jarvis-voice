...
def test_local_kokoro_voice_override_changes_request_and_cache_key(tmp_path):
    script, env, curl_args, _, _ = _isolated_local_status_script(tmp_path, "kokoro")
    config = script.parent.parent / "config" / "local.env"
    config.write_text(
    ...
    cache_dir = Path(env["HOME"]) / ".cache" / "jarvis" / "status-tts-local"
