...
from serpapi_client import (  # noqa: E402
...
from flight_search import convert_fallback_itinerary, main, sort_results  # noqa: E402
...
class FallbackConversionTests(unittest.TestCase):
    ...
    def _fake_fast_flights(self, get_flights):
        module = ModuleType("fast_flights")
        captured = {"legs": [], "queries": []}

        class FlightQuery:
            def __init__(self, **kwargs):
                self.kwargs = kwargs
                captured["legs"].append(kwargs)
        ...
        module.FlightQuery = FlightQuery
        module.Passengers = Passengers
        module.create_query = create_query
        module.get_flights = get_flights
    ...
    def test_only_truly_unapplied_filters_are_reported(self):
        """max_price and include_airlines are applied, so they are not reported."""
    ...
    def test_fallback_encodes_airline_codes_and_stop_limit_in_each_leg(self):
        fake_module, captured = self._fake_fast_flights(lambda query, proxy=None: [self._entry()])
        ...
        with patch.dict(sys.modules, {"fast_flights": fake_module}), patch(
            "flight_search.build_proxy_url_attempts", return_value=[None]
        ):
            results, meta = flight_search.search_fallback(options)

        self.assertEqual(len(results), 1)
        self.assertEqual(len(captured["legs"]), 2)
        for leg in captured["legs"]:
            self.assertEqual(leg["airlines"], ["AS", "AA"])
            self.assertEqual(leg["max_stops"], 0)
        self.assertEqual(meta["serpapi_searches_used"], 0)

    def test_fallback_retries_proxy_chain_without_leaking_proxy_credentials(self):
        calls = []

        def fail_every_attempt(query, proxy=None):
            calls.append(proxy)
            raise RuntimeError(f"failed via {proxy}")

        fake_module, _ = self._fake_fast_flights(fail_every_attempt)
        secret_proxy = "http://secret-user:secret-pass@proxy.example.test:8080"
        with patch.dict(sys.modules, {"fast_flights": fake_module}), patch(
            "flight_search.build_proxy_url_attempts", return_value=[secret_proxy, None]
        ):
            with self.assertRaises(RuntimeError) as raised:
                flight_search.search_fallback(self._options())

        self.assertEqual(calls, [secret_proxy, None])
        self.assertNotIn("secret-user", str(raised.exception))
        self.assertNotIn("secret-pass", str(raised.exception))

    def test_fallback_price_cap_excludes_results_with_unknown_prices(self):
        unknown = self._entry()
        unknown.price = None
        expensive = self._entry()
        expensive.price = 257
        fake_module, _ = self._fake_fast_flights(
            lambda query, proxy=None: [unknown, expensive]
        )
        with patch.dict(sys.modules, {"fast_flights": fake_module}), patch(
            "flight_search.build_proxy_url_attempts", return_value=[None]
        ):
            results, _ = flight_search.search_fallback(self._options(max_price=200))
