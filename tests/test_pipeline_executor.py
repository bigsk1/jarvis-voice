...
class PipelineExecutorResolutionTests(unittest.TestCase):
    ...
    def test_google_local_services_uses_workflow_search_url_extraction(self):
        variables = {}
        result = {
            "ok": True,
            "data": {
                "results": [
                    {"url": "https://services.example/provider-one"},
                    {"url": "https://services.example/provider-two"},
                ]
            },
        }

        self.assertTrue(
            self.executor._is_search_tool("serpapi_google_local_services")
        )
        self.executor._apply_output_transforms(
            {}, result, variables, "serpapi_google_local_services", None
        )
        self.assertEqual(
            variables["search_results"]["urls"],
            [
                "https://services.example/provider-one",
                "https://services.example/provider-two",
            ],
        )
...
"""
    ...
    def test_jarvis_self_check_uses_health_issues_alerts_and_single_canvas_page(self):
        ...
        self.assertEqual(health_step["extract"]["alert_severity"], "alert_severity")
        self.assertEqual(health_step["extract"]["alert_dedupe_key"], "alert_dedupe_key")
        self.assertLess(step_tools.index("canvas"), step_tools.index("create_alert"))
        ...
        self.assertEqual(alert_step["params"]["severity"], "${alert_severity}")
        self.assertEqual(alert_step["params"]["metadata"]["dedupe_key"], "${alert_dedupe_key}")
        self.assertTrue(alert_step["required"])
        self.assertEqual(canvas_actions, ["read", "create", "update"])
        self.assertEqual(canvas_steps[1]["condition"]["op"], "not_exists")
        self.assertEqual(canvas_steps[2]["condition"]["op"], "exists")
        self.assertTrue(all(step["required"] for step in canvas_steps))
        self.assertTrue(canvas_steps[2]["params"]["allow_content_shrink"])
        self.assertIn("## Current Status", canvas_steps[2]["params"]["content"])
        self.assertIn("## Active Issues", canvas_steps[2]["params"]["content"])
        self.assertIn("## Alert Policy", canvas_steps[2]["params"]["content"])
        self.assertIn("## Raw Health Snapshot", canvas_steps[2]["params"]["content"])
