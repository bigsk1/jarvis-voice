...
3. On a public test page, click the toolbar to grant temporary tab access. Capture the page and inspect the screenshot and full-text previews. Save the reviewed text to Library, confirm its URL and capture date, and check that no chat message or screenshot was sent by that action. Capture and save again after a page change; the earlier source should remain. Then send a separate capture through chat. Unsent previews can be removed without uploading them.
...
7. With Jarvis Voice 2.55.8 or newer and a configured task worker, start an allowed background task in Jarvis Web and open the same conversation in the companion. Its card and late answer should appear without settling an unrelated active turn or triggering Talk playback. Reopen history to recover the saved result. Firefox tool calls remain foreground; task settings and cancellation stay in Jarvis Web. Web-started jobs do not trigger the extension's request-completion notifications.
